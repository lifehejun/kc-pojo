package com.kc.manage.interceptor;
...
public class CommonInterceptor extends HandlerInterceptorAdapter {  
    ...
    public boolean preHandle(HttpServletRequest request,    
		...
		AdminUserBean user = (AdminUserBean) request.getSession().getAttribute("user");
		if (user == null) {
		...
		//加载配置信息表
		List<BusConfig> busConfigList = busConfigService.findList();
		Map<String, List<BusConfig>> busConfigMapList = busConfigList.stream().collect(Collectors.groupingBy(BusConfig::getBusType));
		Map<String, Map<String,BusConfig>> busConfigMap = new HashMap<>();
		for (String type:busConfigMapList.keySet()){
			List<BusConfig> nameConfigList = busConfigMapList.get(type);
			Map<String, BusConfig> groupMap = nameConfigList.stream().collect(Collectors.toMap(BusConfig::getName, Function.identity()));
			busConfigMap.put(type,groupMap);
		}
		request.setAttribute("busConfig",busConfigMap);
    	return true;
    }    
